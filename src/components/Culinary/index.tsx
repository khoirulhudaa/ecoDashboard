...
const Culinary: React.FC = () => {
  ...
  const [dataCulinaries, setDataCulinaries] = useState<any[]>([])
  const [search, setSearch] = useState<string>('')

  useEffect(() => {

    (async () => {
      const response = await API.getAlCulinary()
      setDataCulinaries(response?.data?.data)
  ...
  const handleRemoveFinally = async (id: string) => {
    console.log(id)
    const response = await API.removeCulinary(id)
    console.log(response)
    if(response?.data?.status === 200) {
      setStatus(true)
      SweetAlert({
        title: 'Berhasil hapus kuliner!',
        showCancelButton: false
      })
    }
  }

  const handleRemoveCulinary = (id: string) => {
    console.log(id)
    SweetAlert({
      title: 'Yakin hapus kuliner ?',
      icon: 'question',
      onClick: () => handleRemoveFinally(id)
    })
  }

  const handleUpdateCulinary = (data?: any) => {
    dispatch(getCulinary(data))
    navigate.push('/update-culinary')
  }
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          <form action="https://formbold.com/s/unique_form_id" method="POST">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Cari kuliner sekarang..."
      ...
      <div className="grid border-t border-stroke px-4 py-4.5 dark:border-strokedark grid-cols-8 md:px-6 2xl:px-7.5">
        ...
        <div className="col-span-4 md:col-span-2 flex items-center">
          <p className="font-medium">Nama kuliner</p>
        </div>
          ...
          </div>
          {
            Auth?.role === 'super-admin' ? (
              <div className="col-span-4 md:col-span-1 flex">
                <div className='flex'>
                  <div onClick={() => handleRemoveCulinary(data?.culinary_id)} className='w-[34px] h-[34px] rounded-[6px] mr-2 bg-[red] cursor-pointer hover:brightness-[90%] active:scale-[0.98] p-1 text-white flex items-center justify-center'>
                    <FaTrash />
                  </div>
                  <div onClick={() => handleUpdateCulinary(data)} className='w-[34px] h-[34px] bg-yellow-500 rounded-[6px] ml-2 cursor-pointer hover:brightness-[90%] active:scale-[0.98] p-1 text-white flex items-center justify-center'>
                    <FaPenAlt />
                  </div>
                </div>
              </div>
